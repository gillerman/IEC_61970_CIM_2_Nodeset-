...
<xsl:stylesheet version="2.0" xmlns="http://opcfoundation.org/UA/2011/03/UANodeSet.xsd" 
	...
	<xsl:template match="/">
        ...
        <Uri>http://gcs.com/IEC61970Types</Uri>
				<Uri>http://gcs.com/IEC61970Instances</Uri>
			</NamespaceUris>
			<Aliases>
...
<!-- *************************** -->
      ...
      <xsl:for-each select="rdf:RDF/cim:Terminal">
        <UAObject>
          ...
          <References>
            ...
            <Reference>
              <xsl:attribute name="ReferenceType">ns=1;s=T.CE</xsl:attribute>
              <xsl:attribute name="IsForward">true</xsl:attribute>
        ...
        <xsl:for-each select="rdf:RDF/cim:CurrentLimit">
          <UAObject>
            ...
            <References>
              ...
              <Reference>
                <xsl:attribute name="ReferenceType">ns=1;s=CL.OLT</xsl:attribute>
                <xsl:attribute name="IsForward">true</xsl:attribute>
                <xsl:sequence select="fn:concat('ns=2;s=', fn:substring(cim:OperationalLimit.OperationalLimitType/@rdf:resource,2,33))"/>
              </Reference>
              <Reference>
                <xsl:attribute name="ReferenceType">ns=1;s=CL.OLS</xsl:attribute>
                <xsl:attribute name="IsForward">true</xsl:attribute>
      ...
      <xsl:for-each select="rdf:RDF/cim:ActivePowerLimit">
        <UAObject>
          ...
          <References>
            ...
            <Reference>
              <xsl:attribute name="ReferenceType">ns=1;s=APL.OLT</xsl:attribute>
              <xsl:attribute name="IsForward">true</xsl:attribute>
              <xsl:sequence select="fn:concat('ns=2;s=', fn:substring(cim:ActivePowerLimit.OperationalLimitType/@rdf:resource,2,33))"/>
            </Reference>
            <Reference>
              <xsl:attribute name="ReferenceType">ns=1;s=APL.OLS</xsl:attribute>
              <xsl:attribute name="IsForward">true</xsl:attribute>
    ...
    <xsl:param name="UAVariableValue"/> 
      ...
      <xsl:attribute name="BrowseName" namespace="" select="fn:concat('2:', @rdf:ID, $UAVariableId)"/>
